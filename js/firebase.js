...
async function upgradeToPro(planType) {
  ...
  // Initialize Razorpay
  const options = {
    key: 'REPLACE_WITH_LIVE_KEY_FROM_SERVER', // Never hardcode — use Worker /create-order
    amount: plan.amount,
    currency: 'INR',
    name: 'MyRight',
    ...
    handler: async (response) => {
      // DEPRECATED: This payment flow is no longer used.
      // All payments now go through the Cloudflare Worker /create-order + /verify-payment.
      // This function is dead code — kept only for reference.
      showToast('Payment flow outdated. Please use the Subscription page.', 'error');
    }
  };
