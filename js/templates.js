// ── DOCUMENT TEMPLATES ───────────────────────────────────────────────────────
// Legal text templates for all 100+ documents

// ── HELPERS ───────────────────────────────────────────────────────────────────
function toWords(n){
  const a=['','One','Two','Three','Four','Five','Six','Seven','Eight','Nine','Ten','Eleven','Twelve','Thirteen','Fourteen','Fifteen','Sixteen','Seventeen','Eighteen','Nineteen'];
  const b=['','','Twenty','Thirty','Forty','Fifty','Sixty','Seventy','Eighty','Ninety'];
  if(!n||isNaN(n))return'';n=parseInt(n);if(n===0)return'Zero';if(n<0)return'Minus '+toWords(-n);
...
function amt(n){return n&&n!="0"?`Rs.${parseInt(n).toLocaleString("en-IN")}/- (Rupees ${toWords(n)} Only)`:"___";}
function fmtDate(d){if(!d)return"___";const x=new Date(d+"T00:00:00");return x.toLocaleDateString("en-IN",{day:"2-digit",month:"long",year:"numeric"});}
function v(x){return x&&x.trim()?x.trim():"___";}
...
function generateDoc(id,f){
  const fn=TMPL[id];
  if(fn){try{const r=fn(f);if(r&&r.trim())return r.trim();}catch(e){console.error(id,e);}}
  const lines=Object.entries(f).filter(([k,val])=>val&&val.trim()).map(([k,val])=>k.replace(/([A-Z])/g,' $1').trim()+': '+val).join('\n');
  return id.replace(/_/g,' ').toUpperCase()+'\n\nDate: '+todayFmt+'\n\n'+lines+'\n\n[Document generated by MyRight. Please have this reviewed by a qualified advocate before use.]\n\nSignature: _______________\nDate: _______________';
}
...
1. I am the deponent above named and I am fully competent to swear to this affidavit. I am making this affidavit from my personal knowledge.
...
4. I state that I am making this affidavit voluntarily, without any force, coercion, misrepresentation or undue influence from any person whatsoever.

5. I state that I shall be fully responsible for any consequences that may arise if any of the facts stated above are found to be incorrect or false.
...
name_change:f=>`AFFIDAVIT FOR CHANGE OF NAME

STATE OF KERALA — DISTRICT OF ${v(f.city).toUpperCase()}

I, ${v(f.oldName)}, son/daughter of ${v(f.fatherName)}, aged ${v(f.age)} years, residing at ${v(f.address)}, ${v(f.city)}, do hereby solemnly affirm and state on oath as follows:
...
2. That my present name as recorded in all official documents and records is: ${v(f.oldName)}.

3. That I desire to be hereafter known, identified and referred to by my new name: ${v(f.newName)}.

4. The reason for this change of name is: ${v(f.reason)}.

5. That with effect from the date of this affidavit, I hereby absolutely, irrevocably and permanently:
   (a) Abandon and relinquish the use of my former name: ${v(f.oldName)}.
   (b) Adopt and assume my new name: ${v(f.newName)}.
   (c) Declare that I shall henceforth be known, identified and referred to only by my new name: ${v(f.newName)} in all matters — legal, official, professional, educational, personal and social.

6. I hereby request all concerned authorities, government departments, courts, banks, financial institutions, insurance companies, educational institutions, employers and all other persons and organisations to:
   (a) Record and recognise my new name: ${v(f.newName)} in place of my old name: ${v(f.oldName)}.
   (b) Make necessary corrections in all existing records accordingly.
   (c) Issue all future documents, certificates and communications in my new name.

7. Documents requiring change include: Aadhaar Card, PAN Card, Passport, Voter ID, Driving License, Bank Accounts, Insurance Policies, Educational Certificates, and all other official records.

8. I declare that this name change is not being made for any fraudulent, illegal or improper purpose.

VERIFICATION: Verified at ${v(f.city)} on ${fmtDate(f.date)}.

                    ________________________
                    Deponent: ${v(f.newName)}
                    (Formerly: ${v(f.oldName)})

Solemnly affirmed before me on ${fmtDate(f.date)}.
________________________
Notary Public (Seal & Stamp) — Reg. No: _______________`,

rent_agreement:f=>{
  const end=new Date(f.startDate);end.setMonth(end.getMonth()+(parseInt(f.duration)||11));
  return `RENT / TENANCY AGREEMENT

This Rent Agreement is made and executed on ${fmtDate(f.startDate)}, at ${v(f.city)}, Kerala.

BETWEEN

LANDLORD:
Name: ${v(f.landlordName)}
S/o D/o: ${v(f.landlordFather||'___')}
Address: ${v(f.landlordAddress||f.city)}
...
Address: ${v(f.tenantAddress||f.city)}
(hereinafter referred to as "the Tenant")

PREMISES: ${v(f.propertyAddress)}, ${v(f.city)}, Kerala.
Property Type: ${v(f.propertyType||'Residential Premises')}

WHEREAS the Landlord is the absolute owner of the above Premises and the Tenant desires to take the same on rent on the terms and conditions set forth herein.

1. TENANCY AND DURATION
   This tenancy shall be for ${v(f.duration)} months from ${fmtDate(f.startDate)} to ${fmtDate(end.toISOString().split('T')[0])}.

2. RENT
   Monthly rent: ${amt(f.rentAmount)}, payable on or before the 5th of each calendar month. Late payment shall attract interest at 18% per annum.

3. SECURITY DEPOSIT
   The Tenant has paid a refundable security deposit of ${amt(f.deposit||'0')}. This shall be refunded within 30 days of vacating after deducting lawful dues.

4. USE OF PREMISES
   The Premises shall be used solely for residential purposes only. No commercial, illegal or offensive use is permitted.

5. MAINTENANCE AND REPAIRS
   (a) Minor day-to-day repairs: Tenant's responsibility.
   (b) Major structural repairs: Landlord's responsibility.
   (c) No structural alterations without prior written consent of Landlord.
   (d) Tenant shall handover the Premises in its original condition at end of tenancy.

6. UTILITIES
   All charges for electricity, water, internet, cable TV and other utilities shall be paid by the Tenant directly.

7. SUBLETTING
   The Tenant shall not sublet, assign or part with possession of the Premises without prior written consent of the Landlord.

8. NOTICE PERIOD
   Either party may terminate this agreement by giving one month's prior written notice.

9. RENEWAL
   At the expiry of this agreement, the tenancy may be renewed by mutual written consent on terms to be agreed, with rent revision by a maximum of 10%.

10. GOVERNING LAW
    This agreement is governed by the Transfer of Property Act, 1882, and applicable laws. Jurisdiction: Courts at ${v(f.city)}, Kerala.

IN WITNESS WHEREOF, both parties sign this agreement on the date first written above.
...
loan_agreement:f=>`LOAN AGREEMENT

This Loan Agreement is made on ${fmtDate(f.date)}, at ${v(f.city)}, Kerala.

BETWEEN

LENDER: ${v(f.lenderName)}, Address: ${v(f.lenderAddress||f.city)}

AND

BORROWER: ${v(f.borrowerName)}, Address: ${v(f.borrowerAddress||f.city)}

WHEREAS the Borrower has requested the Lender for a loan and the Lender has agreed to lend the same on the terms and conditions set forth herein.

1. LOAN AMOUNT
   The Lender agrees to lend to the Borrower: ${amt(f.loanAmount)}.

2. DISBURSEMENT
   The loan is disbursed on the date of this Agreement by cash / bank transfer. The Borrower acknowledges receipt of the full loan amount.

3. RATE OF INTEREST
   ${parseInt(f.interest||0)>0?`The Borrower shall pay interest at ${v(f.interest)}% (${toWords(parseInt(f.interest))} percent) per annum on the outstanding amount, calculated on a monthly reducing balance.`:`This loan is advanced as an interest-free loan.`}

4. REPAYMENT
   The Borrower shall repay the entire outstanding amount on or before ${fmtDate(f.repaymentDate)}.
   Mode of Repayment: ${v(f.repaymentMode||'As mutually agreed')}.
   ${f.security&&f.security.trim()?'Security / Collateral: '+v(f.security):''}

5. PREPAYMENT
   The Borrower may prepay the loan in part or full at any time without penalty.

6. DEFAULT
   (a) In case of default, the entire outstanding amount becomes immediately due and payable.
   (b) Default shall attract penal interest at ${Math.min(parseInt(f.interest||0)+6,24)}% per annum from the date of default.
   (c) All recovery costs shall be borne by the Borrower.

7. GOVERNING LAW
   This agreement is governed by and construed in accordance with the Indian Contract Act, 1872, and all other applicable laws of India. Any dispute shall be subject to the exclusive jurisdiction of the courts at ${v(f.city)}, Kerala.
...
This Non-Disclosure Agreement is made on ${fmtDate(f.date)}, at ${v(f.city)}.

BETWEEN

DISCLOSING PARTY: ${v(f.party1)}, Address: ${v(f.party1Address||f.city)}

AND

RECEIVING PARTY: ${v(f.party2)}, Address: ${v(f.party2Address||f.city)}

PURPOSE: ${v(f.purpose)}

WHEREAS the Disclosing Party possesses certain confidential and proprietary information and desires to share the same with the Receiving Party for the above Purpose, the Receiving Party agrees to maintain strict confidentiality.

1. DEFINITION OF CONFIDENTIAL INFORMATION
   "Confidential Information" means any and all information, data, know-how, trade secrets, business strategies, financial data, technical specifications, customer data, pricing, software, processes, inventions, or any other information disclosed by the Disclosing Party — whether in writing, orally, electronically or otherwise — which is designated as confidential or which by its nature should reasonably be understood to be confidential.
...
2. OBLIGATIONS OF RECEIVING PARTY
   ...
   (d) Restrict access to need-to-know personnel only, bound by equivalent confidentiality obligations;
   (e) Protect the Confidential Information with at least the same degree of care as its own confidential information.
...
3. EXCLUSIONS
   ...
   (d) Is required to be disclosed by applicable law or court order (with prior notice to Disclosing Party).

4. NO GRANT OF RIGHTS
   Nothing herein grants any license, title or intellectual property rights in the Confidential Information.

5. RETURN OF INFORMATION
   Upon request or termination, the Receiving Party shall return or securely destroy all Confidential Information.

6. TERM
   This Agreement is effective for ${v(f.duration)} months from the date hereof. Confidentiality obligations survive termination for 3 years.

7. REMEDIES
   A breach shall cause irreparable harm. The Disclosing Party is entitled to seek injunctive relief in addition to all other remedies at law.

8. GOVERNING LAW
   Governed by Indian Contract Act, 1872 and Information Technology Act, 2000. Jurisdiction: Courts at ${v(f.jurisdiction||f.city)}.
...
[Note: This NDA should be executed on non-judicial stamp paper of appropriate value. For transactions above Rs.1,00,000 in value or as required by applicable laws, get this document notarised.]`,
...
employment_contract:f=>`EMPLOYMENT CONTRACT

This Employment Contract is made on ${fmtDate(f.joiningDate)}, at ${v(f.city)}.

BETWEEN

EMPLOYER: ${v(f.employerName)}, Address: ${v(f.companyAddress||f.city)}

AND

EMPLOYEE: ${v(f.employeeName)}

WHEREAS the Employer desires to employ the Employee as ${v(f.designation)} and the Employee accepts employment on the terms set forth herein.

1. APPOINTMENT AND COMMENCEMENT
   The Employee is appointed as ${v(f.designation)}${f.department?' in the '+v(f.department)+' Department':''} with effect from ${fmtDate(f.joiningDate)}.

2. PROBATIONARY PERIOD
   The Employee shall be on probation for ${v(f.probation||'3')} months. Confirmation is subject to satisfactory performance. The Employer may, at its discretion, extend the probation period.

3. COMPENSATION
   Monthly Salary: ${v(f.salary)}
   Structure: ${v(f.salaryStructure||'As agreed')}

4. WORKING HOURS
   ${v(f.workingHours||'As per Company policy')}. The Employee may be required to work beyond normal hours when required.

5. LEAVE
   ${v(f.leaveEntitlement||'As per Company leave policy')}.

6. CONFIDENTIALITY
   During and after employment, the Employee shall maintain strict confidentiality of all proprietary information including trade secrets, customer data, financial information, business strategies, software, processes and pricing. This obligation survives termination of employment indefinitely.

7. INTELLECTUAL PROPERTY
   All inventions, works, developments, discoveries and other intellectual property created by the Employee during employment shall be the exclusive property of the Employer. The Employee agrees to execute all documents necessary to vest such rights in the Employer.

8. CODE OF CONDUCT
   The Employee shall at all times: (a) Act with honesty, integrity and professionalism; (b) Comply with all Company policies and regulations; (c) Avoid conflicts of interest; (d) Not accept bribes or improper benefits; (e) Not engage in harassment or discrimination.

9. NOTICE PERIOD AND TERMINATION
   (a) Either party may terminate by giving ${v(f.noticePeriod||'30')} days written notice, or salary in lieu thereof.
   (b) Immediate termination (without notice) for: gross misconduct, fraud, dishonesty, willful insubordination, or serious breach.

10. NON-SOLICITATION
    For 12 months after cessation of employment, the Employee shall not solicit the Employer's clients or employees.

11. GOVERNING LAW
    This Contract is governed by Indian employment laws including the Industrial Disputes Act, 1947, and applicable Shops and Establishments Act. Jurisdiction: ${v(f.city)}.

________________________     ________________________
For ${v(f.employerName)}     EMPLOYEE
Authorised Signatory         ${v(f.employeeName)}
Date: ${fmtDate(f.joiningDate)}  Date: ${fmtDate(f.joiningDate)}
...
FROM: ${v(f.senderName)}, ${v(f.senderAddress)}, ${v(f.city)}
BY: REGD. POST A.D. / SPEED POST
Date: ${fmtDate(f.date)}   Ref. No: LN/${new Date().getFullYear()}/${RN()}

TO,
${v(f.receiverName)}
${v(f.receiverAddress)}

SUBJECT: LEGAL NOTICE — ${v(f.demand).substring(0,80).toUpperCase()}

TAKE NOTICE THAT:

1. I, ${v(f.senderName)}, send you this Legal Notice in my own right, reserving all rights and remedies expressly.

2. FACTS:
${v(f.noticeReason)}

3. LEGAL POSITION:
   Your acts and omissions constitute a clear breach of your legal obligations under Indian law, including the Indian Contract Act, 1872, the Bharatiya Nyaya Sanhita (BNS) 2023, and all other applicable statutes. Your conduct has caused me substantial loss and injury.

4. DEMAND:
   I hereby call upon you to, within ${v(f.deadline)} (${toWords(parseInt(f.deadline)||15)}) days from the date of receipt of this Notice:
   ${v(f.demand)}

5. CONSEQUENCE OF NON-COMPLIANCE:
   TAKE FURTHER NOTICE THAT if you fail to comply with the above demand within the stated period, I shall be constrained to initiate appropriate civil, criminal and/or consumer proceedings against you, entirely at your risk, cost and consequences. The costs of this notice and all future legal proceedings shall be recovered from you.

This Notice is issued without prejudice to all my other rights and remedies, all of which are expressly reserved.
...
cheque_bounce_notice:f=>`LEGAL NOTICE UNDER SECTION 138 OF THE NEGOTIABLE INSTRUMENTS ACT, 1881
...
SUBJECT: NOTICE U/S 138 NI ACT — DEMAND FOR PAYMENT OF DISHONOURED CHEQUE

TAKE NOTICE THAT:

1. You had issued Cheque No. ${v(f.chequeNo)}, dated ${fmtDate(f.chequeDate)}, drawn on ${v(f.bank)} for ${amt(f.chequeAmount)}, in discharge of a legally enforceable debt / liability due from you to me.

2. When the said cheque was presented for encashment, it was returned / dishonoured by the bank on ${fmtDate(f.dishonourDate)} with the remark: "${v(f.dishonourReason)}".

3. The dishonour of the cheque constitutes an offence under Section 138 of the Negotiable Instruments Act, 1881 (as amended), punishable with imprisonment up to 2 years and/or fine up to twice the cheque amount.

4. I hereby DEMAND that you pay me the sum of ${amt(f.chequeAmount)}, together with interest and costs, within 15 (Fifteen) days from the date of receipt of this Notice.

5. TAKE FURTHER NOTICE THAT if you fail to make payment within 15 days, I shall file a criminal complaint against you under Section 138 NI Act before the competent Magistrate Court, without any further notice. You shall also be liable for a civil suit to recover the amount with interest.

                    ________________________
                    ${v(f.senderName)}`,

consumer_complaint:f=>`CONSUMER COMPLAINT LETTER

FROM: ${v(f.complainantName)}, ${v(f.complainantAddress)}
Date: ${fmtDate(f.date)}

TO,
The Customer Grievance / Legal Department
${v(f.respondentName)}
${v(f.respondentAddress)}

SUBJECT: CONSUMER COMPLAINT — DEFICIENCY IN GOODS / SERVICES — DEMAND FOR REDRESSAL

Dear Sir / Madam,

1. I am a consumer as defined under Section 2(7) of the Consumer Protection Act, 2019.

2. On ${fmtDate(f.purchaseDate)}, I purchased / availed: ${v(f.productService)}, paying ${amt(f.amountPaid)}.

3. NATURE OF COMPLAINT:
${v(f.deficiency)}

4. Despite raising complaints with your representatives, no satisfactory resolution has been provided. This constitutes a clear deficiency in service / goods under Section 2(11) of the Consumer Protection Act, 2019.

5. DEMAND:
I hereby demand that within 15 days of receiving this letter, you:
${v(f.relief)}

6. If you fail to redress my grievance as above, I shall file a complaint before the District Consumer Disputes Redressal Commission under Section 34 of the Consumer Protection Act, 2019, and claim additional compensation for mental agony and harassment. All legal costs shall be claimed from you.

Yours faithfully,
________________________
${v(f.complainantName)}`,

poa_general:f=>`GENERAL POWER OF ATTORNEY

I, ${v(f.principalName)}, residing at ${v(f.principalAddress)}, ${v(f.city)}, hereby appoint ${v(f.agentName)}, ${v(f.agentRelation)}, residing at ${v(f.agentAddress)}, as my true and lawful Attorney.

POWERS GRANTED:
${v(f.powers)}

Additionally, my Attorney is authorised to do all acts, deeds, and things as may be necessary for the above purposes, as fully and effectually as I could do if personally present.

This Power of Attorney shall ${f.validUntil ? 'remain valid until '+fmtDate(f.validUntil) : 'remain in force until revoked in writing'}.

IN WITNESS WHEREOF, I, ${v(f.principalName)}, have hereunto set my hand at ${v(f.city)} on this ${fmtDate(f.date)}.
...
[Note: This General Power of Attorney should be executed on non-judicial stamp paper of appropriate value and notarised by a Notary Public. For immovable property transactions, registration before the Sub-Registrar is mandatory.]`,

// ═══════════════════════════════════════════════════════════════
// PHASE 1 TEMPLATES — PERSONAL DOCUMENTS (10 docs)
// ═══════════════════════════════════════════════════════════════
...
property_sale:f=>`AGREEMENT TO SELL
(Under Section 54 of the Transfer of Property Act, 1882)

This Agreement to Sell (hereinafter referred to as "the Agreement") is made and executed on ${fmtDate(f.date)}, at ${v(f.city)}, Kerala.

BETWEEN

VENDOR / SELLER:
Name: ${v(f.sellerName)}
S/o D/o W/o: ${v(f.sellerFather)}
Address: ${v(f.sellerAddress)}
...
PURCHASER / BUYER:
Name: ${v(f.buyerName)}
S/o D/o W/o: ${v(f.buyerFather)}
Address: ${v(f.buyerAddress)}
...
RECITALS:
WHEREAS the Vendor is the absolute owner of the Schedule Property by virtue of lawful title, and has represented that the property is free from all encumbrances, charges, litigation, mortgage, lien, attachments and claims of any nature whatsoever, AND the Purchaser has agreed to purchase the Schedule Property from the Vendor on the terms and conditions set forth herein.

NOW THIS AGREEMENT WITNESSES AS FOLLOWS:

1. SALE CONSIDERATION
   The total agreed sale consideration for the Schedule Property is:
   ${amt(f.saleAmount)}
   (Rupees ${toWords(parseInt(f.saleAmount||0))} Only)

2. PAYMENT OF CONSIDERATION
   (a) Advance / Token amount paid by the Purchaser on the date of this Agreement:
       ${amt(f.advanceAmount||'0')}
   (b) Balance consideration payable on or before execution and registration of the Sale Deed:
       Rs. ${(parseInt(f.saleAmount||0) - parseInt(f.advanceAmount||0)).toLocaleString('en-IN')}/-
   (c) Mode of payment: Demand Draft / Account Payee Cheque / RTGS / NEFT / as mutually agreed.

3. TITLE AND POSSESSION
   (a) The Vendor warrants that the Schedule Property has a clear, marketable and absolute title.
   (b) The Vendor shall hand over vacant and peaceful possession of the Schedule Property to the Purchaser simultaneously with the execution and registration of the Sale Deed on or before ${fmtDate(f.possessionDate)}.
   (c) The Vendor shall deliver all original title deeds, encumbrance certificates, tax receipts, and all other documents relating to the property to the Purchaser at the time of registration.

4. OBLIGATIONS OF VENDOR
   (a) To execute the Sale Deed within the agreed time on receipt of balance consideration.
   (b) To clear all outstanding electricity, water, property tax and other dues up to the date of handing over possession.
   (c) To obtain all necessary NOCs and clearances from concerned authorities.
   (d) To not create any encumbrance, mortgage or third-party interest on the property between the date of this Agreement and the date of registration.
   (e) To obtain any necessary permissions from housing society, government or other statutory authorities.

5. OBLIGATIONS OF PURCHASER
   (a) To pay the balance sale consideration on or before the stipulated date.
   (b) To bear all stamp duty, registration fees, legal fees and other incidental charges for registration of the Sale Deed.
   (c) To pay property tax and all other outgoings from the date of possession.

6. DEFAULT BY VENDOR
   If the Vendor fails or refuses to execute the Sale Deed after receiving the full consideration, the Purchaser shall be entitled to:
   (a) Seek specific performance of this Agreement under Section 10 of the Specific Relief Act, 1963; and / or
   (b) Recover double the advance amount paid; and / or
   (c) Recover all damages and costs incurred.

7. DEFAULT BY PURCHASER
   If the Purchaser fails to pay the balance consideration or complete the purchase by the stipulated date, the Vendor shall be entitled to forfeit the advance amount paid and treat this Agreement as cancelled, after giving 15 days written notice.

8. TIME IS OF THE ESSENCE
   Time shall be of the essence in this Agreement. All parties shall strictly comply with the time limits specified herein.

9. ENCUMBRANCES
   The Vendor shall clear all outstanding loans, mortgages and dues on the Schedule Property before or at the time of registration of the Sale Deed.

10. GOVERNING LAW AND JURISDICTION
    This Agreement is governed by the Transfer of Property Act, 1882, the Registration Act, 1908, the Specific Relief Act, 1963, and the Indian Stamp Act, 1899. All disputes shall be subject to the jurisdiction of courts at ${v(f.city)}, Kerala.

SCHEDULE OF PROPERTY:
${v(f.propertyAddress)}, ${v(f.city)}, Kerala

IN WITNESS WHEREOF, both parties have signed this Agreement on the date first above written.
...
[Note: This Agreement to Sell does NOT transfer ownership of the property. Ownership transfers only upon execution and registration of the Sale Deed. Stamp duty as per the Indian Stamp Act / Kerala Stamp Act is payable on this Agreement.]`,
...
promissory_note:f=>`PROMISSORY NOTE

Place: ${v(f.city)}, Kerala
Date: ${fmtDate(f.date)}
Amount: ${amt(f.amount)}

FOR VALUE RECEIVED, I / We, ${v(f.makerName)}, residing at ${v(f.city)}, Kerala (hereinafter referred to as "the Maker / Promisor"), hereby unconditionally and irrevocably promise to pay to ${v(f.payeeName)} (hereinafter referred to as "the Payee / Holder"), or order, on demand / on ${fmtDate(f.dueDate)}, the sum of:

${amt(f.amount)}
(Rupees ${toWords(parseInt(f.amount||0))} Only)

For value received as: ${v(f.purpose)}.

INTEREST:
${parseInt(f.interest||0) > 0
  ? `This Promissory Note carries interest at the rate of ${v(f.interest)}% (${toWords(parseInt(f.interest))} percent) per annum from the date hereof until the date of full payment. Interest shall be calculated on a monthly reducing balance basis.`
  : `This Promissory Note is interest-free. No interest shall accrue on the principal amount.`}

DEFAULT INTEREST:
In the event of failure to pay the principal and / or interest on the due date, the outstanding amount shall carry additional default interest at the rate of ${Math.min(parseInt(f.interest||0)+6, 24)}% per annum from the date of default until the date of actual full payment.

PRESENTMENT FOR PAYMENT:
Presentment for payment is waived. The Payee / Holder shall be entitled to demand payment at any time on or after the due date without prior notice.

COSTS OF RECOVERY:
In the event of any default, the Maker shall bear and pay all costs, charges, legal fees and expenses incurred by the Payee / Holder in recovering the outstanding amount.

NEGOTIABILITY:
This Promissory Note is governed by the Negotiable Instruments Act, 1881 and is a negotiable instrument. The Payee may endorse and transfer this Note to any third party.

GOVERNING LAW:
This Note is governed by the Negotiable Instruments Act, 1881 and the Indian Contract Act, 1872. Jurisdiction: Courts at ${v(f.city)}, Kerala.

                                        ________________________
                                        Signature of Maker / Promisor
                                        Name: ${v(f.makerName)}
                                        Date: ${fmtDate(f.date)}
                                        Place: ${v(f.city)}
...
[NOTE: This Promissory Note must be executed on stamp paper of appropriate value as per the Indian Stamp Act, 1899 / Kerala Stamp Act. The stamp duty is 0.5% of the principal amount subject to a maximum of Rs.500/- in Kerala. An unstamped Promissory Note is not admissible as evidence in civil proceedings.]`,
...
This Service Agreement (hereinafter referred to as "the Agreement") is made and executed on ${fmtDate(f.date)}, at ${v(f.city)}, Kerala.

BETWEEN

SERVICE PROVIDER:
Name / Company: ${v(f.serviceProvider)}
Address: ${v(f.providerAddress||f.city)}
...
Address: ${v(f.clientAddress||f.city)}
(hereinafter referred to as "the Client")

WHEREAS the Client desires to engage the Service Provider to perform the services described herein, and the Service Provider agrees to render such services on the terms and conditions set out below.

NOW THIS AGREEMENT WITNESSES AS FOLLOWS:

1. SCOPE OF SERVICES
   The Service Provider agrees to provide the following services to the Client:
   ${v(f.serviceDescription)}
   (hereinafter referred to as "the Services")

2. TERM
   This Agreement shall commence on ${fmtDate(f.startDate)} and shall continue for a period of ${v(f.duration)} months, unless earlier terminated in accordance with this Agreement.

3. SERVICE FEES
   The Client agrees to pay the Service Provider ${amt(f.fees)} as service fees.
   Payment Terms: ${v(f.paymentTerms)}
   All invoices shall be raised by the Service Provider and paid by the Client within 15 days of the invoice date. Delayed payment shall attract interest at 18% per annum.

4. TAXES
   All fees mentioned above are exclusive of applicable Goods and Services Tax (GST). GST shall be charged additionally at the rate applicable from time to time and shall be borne by the Client. The Service Provider shall issue proper GST-compliant invoices.

5. STANDARD OF SERVICE
   The Service Provider shall perform the Services with reasonable skill, care, diligence and in a professional manner consistent with best industry practices.

6. CONFIDENTIALITY
   Both parties shall maintain strict confidentiality of all proprietary, technical, commercial and financial information disclosed during the course of this Agreement. This obligation shall survive the termination of this Agreement for a period of 3 years.

7. INTELLECTUAL PROPERTY
   (a) All pre-existing intellectual property of each party shall remain that party's exclusive property.
   (b) All deliverables, works, inventions and developments created specifically for the Client under this Agreement shall, upon receipt of full payment, become the Client's exclusive property.

8. NON-SOLICITATION
   During the term and for 12 months after termination, neither party shall solicit the other party's employees, clients or contractors without prior written consent.

9. WARRANTIES
   The Service Provider warrants that:
   (a) It has full authority to enter into this Agreement.
   (b) The Services shall be performed in accordance with applicable laws.
   (c) The Services shall not infringe any third-party intellectual property rights.

10. LIMITATION OF LIABILITY
    The Service Provider's total liability under this Agreement shall not exceed the total fees paid by the Client in the 3 months preceding the claim. Neither party shall be liable for indirect, consequential or special damages.

11. TERMINATION
    (a) Either party may terminate this Agreement by giving 30 days prior written notice.
    (b) Either party may terminate immediately on material breach if not remedied within 15 days of written notice.
    (c) On termination, the Client shall pay for all Services rendered up to the termination date.

12. DISPUTE RESOLUTION
    Any dispute shall first be attempted to be resolved by mutual discussion within 30 days. If unresolved, it shall be referred to arbitration under the Arbitration and Conciliation Act, 1996.
...
${v(f.serviceProvider)}           ${v(f.clientName)}
Date: ${fmtDate(f.date)}          Date: ${fmtDate(f.date)}
...
divorce_mutual:f=>`MUTUAL DIVORCE AGREEMENT / MEMORANDUM OF UNDERSTANDING

This Agreement is made on ${fmtDate(f.date)}, at ${v(f.city)}, Kerala.

BETWEEN

HUSBAND: ${v(f.husbandName)}
WIFE: ${v(f.wifeName)}
(hereinafter referred to as "the Parties")

RECITALS:
The Parties were married on ${fmtDate(f.marriageDate)} at ${v(f.marriagePlace)}.
The Parties have been living separately since: ${v(f.separationDetails)}.
The Parties have mutually and amicably decided to dissolve the marriage by way of mutual consent divorce under Section 13B of the Hindu Marriage Act, 1955 / Section 28 of the Special Marriage Act, 1954 / applicable personal law.

This Agreement records the terms mutually agreed upon by the Parties.

1. MUTUAL CONSENT FOR DIVORCE
   Both Parties confirm that they mutually and voluntarily consent to the dissolution of their marriage. Neither party has been subjected to any coercion, fraud or undue influence.

2. PROPERTY SETTLEMENT
   ${v(f.propertySettlement)}

3. CHILD CUSTODY, VISITATION AND MAINTENANCE
   ${f.childCustody && f.childCustody.trim() ? v(f.childCustody) : 'Not applicable — no children from this marriage.'}

4. MAINTENANCE / ALIMONY
   ${parseInt(f.maintenanceAmount||0) > 0
     ? `The ${v(f.husbandName)} agrees to pay maintenance / alimony of ${amt(f.maintenanceAmount)} per month to ${v(f.wifeName)} until her remarriage or further order of the court.`
     : 'Both parties confirm that no maintenance or alimony is payable by either party to the other. Both parties waive their right to claim maintenance from each other.'}

5. OTHER AGREED TERMS
   ${v(f.mutualAgreement)}

6. FULL AND FINAL SETTLEMENT
   Subject to the above terms being complied with, each party releases the other from all claims, demands, rights and liabilities — past, present and future — arising out of or connected with the marriage.

7. PROCEEDINGS
   The Parties shall jointly present the First Motion Petition for Mutual Consent Divorce under Section 13B(1) HMA / applicable section before the Family Court at ${v(f.city)}. After the mandatory cooling-off period (minimum 6 months from First Motion, or as waived by the Court), the Parties shall jointly present the Second Motion Petition to obtain the Divorce Decree.

8. GOVERNING LAW
   This Agreement is governed by the Hindu Marriage Act, 1955 / applicable personal law, and the Family Courts Act, 1984.
...
[NOTE: This Agreement by itself does not dissolve the marriage. It is a pre-filing agreement. The actual divorce requires a Decree from the Family Court after following the procedure under Section 13B of the Hindu Marriage Act, 1955. Both parties must appear in person before the Court.]`,
