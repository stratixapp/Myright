// ===========================
// MYRIGHT v16 — FIREBASE AUTH & DATA LAYER (Production-hardened)
//
// Key security changes from v11:
//   1. upgradePlan() is REMOVED — plan is now written only by the Cloudflare Worker
//      after server-side payment verification. Frontend reads plan from Firestore.
//   2. getPlan() / isPro() fetch from Firestore, not only from localStorage.
//      localStorage is a cache only — it never grants access by itself.
//   3. requireAuth() uses firebase.auth().currentUser for truth, not just localStorage.
//   4. No guest session creation in auth module — legal chat handles its own redirect.
// ===========================
...
// ── Local session cache (UI only — never source of truth for access) ─────────
function _getSession() {
  try { return JSON.parse(localStorage.getItem('mr_session') || 'null'); } catch { return null; }
}
function _saveSession(s) {
  try { localStorage.setItem('mr_session', JSON.stringify(s)); } catch {}
}
...
window.MRAuth = {
  ...
  // Returns cached session for quick UI rendering (not for access control)
  currentUser() {
    return _getSession();
  },
  ...
  async signUp(name, email, password) {
    ...
    try {
      ...
      return { user: session };
    } catch(e) { return { error: _fbError(e.code) }; }
  },
  ...
  async getUserData() {
    ...
    try {
      ...
      if (snap.exists) {
        ...
        // Refresh session cache with latest Firestore values
  ...
  // ── getPlan: reads from Firestore for accuracy; localStorage is cache only ──
  // For access control decisions, always call getUserData() and check .plan there.
  getPlan() {
    const s = _getSession();
    if (!s) return 'free';
    ...
    if (s.planExpiry) {
      try {
        if (new Date(s.planExpiry) < new Date()) {
          // Expired — reset cache; Firestore is source of truth
          s.plan = 'free'; s.planExpiry = null;
          _saveSession(s);
          return 'free';
        }
      } catch(e) {}
  ...
  // ── upgradePlan is REMOVED ────────────────────────────────────────────────
  // Plan upgrades now happen ONLY via the Cloudflare Worker /verify-payment
  // endpoint after server-side Razorpay signature verification.
  // The frontend refreshes its local cache by calling getUserData() after verification.
  async refreshPlanFromFirestore() {
    const session = _getSession();
    if (!session || !_initFB()) return;
    try {
      const snap = await _fbDb.collection('users').doc(session.uid).get();
      if (snap.exists) {
        const data = snap.data();
    ...
    } catch(e) { console.error('refreshPlan error:', e); }
  },

  async updateField(field, value) {
    const session = _getSession();
    if (!session || !_initFB()) return false;
    // Block attempts to update plan or planExpiry from frontend
    if (field === 'plan' || field === 'planExpiry') {
      console.warn('MRAuth.updateField: plan fields are managed by backend only.');
      return false;
    }
    try {
      const update = {};
      ...
      if (field in session) { session[field] = value; _saveSession(session); }
      return true;
    } catch(e) { console.error('updateField error:', e); return false; }
  },
  ...
  // ── requireAuth: checks Firebase Auth state, not just localStorage ───────
  requireAuth() {
    // Quick sync check from cache for immediate redirect (prevents flash)
    const session = _getSession();
    if (!session) {
    ...
    // Async verification: if Firebase says user is not signed in, sign out
    if (_initFB()) {
...
// ── MRPurchases — Firebase-backed (read only from frontend) ──────────────────
window.MRPurchases = {
  async getAll(uid) {
    if (!_initFB()) return [];
    try {
  ...
  async hasPurchased(uid, itemId) {
    if (!_initFB()) return false;
    try {
      // Check by docId or bundleId
      const snap1 = await _fbDb.collection('purchases')
        .where('userId', '==', uid).where('docId', '==', itemId).limit(1).get();
      if (!snap1.empty) return true;
      const snap2 = await _fbDb.collection('purchases')
        .where('userId', '==', uid).where('bundleId', '==', itemId).limit(1).get();
      return !snap2.empty;
    } catch(e) { return false; }
  }
  // addPurchase() removed — purchases are written ONLY by the backend worker
};
...
window.MRAnalytics = {
  ...
  async getTopDocTypes(uid, limit = 4) {
    ...
    try {
      ...
      return Object.entries(snap.data().byType || {})
